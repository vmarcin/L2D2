...
module Edge = struct
  ...
  let compare edge edge' =
    ...
    if phys_equal edge.edge edge'.edge then 0
      ...
      if (Int.equal res 0) then begin
        ...
        if (Int.equal res 0) then begin
          let res = Procname.compare edge.pname edge'.pname in
  ...
  let may_deadlock edge edge' =
    let edge1 = edge.edge in
    let edge2 = edge'.edge in
    if LockEvent.equal (fst edge1) (snd edge2) &&
      LockEvent.equal (snd edge1) (fst edge2) then
      true
    else
      false  
...
module Edges = struct
include AbstractDomain.FiniteSet(Edge)

(* Returns set of all locks used in an analysed program. *)
let get_all_locks : t -> Lockset.t =
  fun dependencies ->
    let set_of_all_locks = Lockset.empty in
    let f : elt -> Lockset.t -> Lockset.t =
      fun pair acc ->
        let first = Lockset.add (fst pair.edge) acc in
        let second = Lockset.add (snd pair.edge) acc in
        Lockset.union first second
    in
    fold f dependencies set_of_all_locks
end
...
let integrate_summary astate callee_pname loc callee_summary callee_formals actuals caller_pname= 
  let formal_to_access_path (name, typ) =
